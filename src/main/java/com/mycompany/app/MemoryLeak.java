...
public class MemoryLeak extends Thread {
  ...
  public MemoryLeak(int bufferSize, WritableByteChannel output, CyclicBarrier doneExit,
  ...
  static private void runTest(TestArguments args) throws InterruptedException, BrokenBarrierException {
    ...
    MemoryLeak[] threads = new MemoryLeak[args.numThreads];
    CyclicBarrier doneExit = new CyclicBarrier(threads.length + 1);
    for (int i = 0; i < threads.length; i++) {
      threads[i] = new MemoryLeak(args.bufferSize, args.devNullChannel, doneExit, args.allocateDirect);
      threads[i].start();
    }
